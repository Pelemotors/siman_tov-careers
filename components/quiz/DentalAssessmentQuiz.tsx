...
export const DentalAssessmentQuiz: React.FC = () => {
  ...
  const handleAnswerSelect = (answerIndex: number) => {
    ...
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', 'dental_quiz_question_answered', {
  ...
  const handleComplete = () => {
    ...
    // Track events
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', 'dental_quiz_completed', {
        event_category: 'engagement',
        event_label: `tier_${tier}`,
      });

      (window as any).gtag('event', `dental_quiz_result_tier_${tier}`, {
        event_category: 'engagement',
  ...
  useEffect(() => {
    if (state.currentQuestionIndex === 0 && !state.isComplete) {
      if (typeof window !== 'undefined' && (window as any).gtag) {
        (window as any).gtag('event', 'dental_quiz_started', {
          event_category: 'engagement',
        });
      }
    }
